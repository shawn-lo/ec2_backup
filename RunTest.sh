...
create_instance()
{
    INSTANCE_ID=$(aws ec2 run-instances --security-groups $EC2_GROUP_NAME --key-name $KEY_NAME --image-id $IMAGE_ID | \
...
connect_instance()
{
    ssh -i $KEY_NAME.pem -o StrictHostKeyChecking=no fedora@$TARGET_IP_ADDRESS "ls -a"
    while [ "$?" != "0" ]
    do
        ssh -i $KEY_NAME.pem -o StrictHostKeyChecking=no fedora@$TARGET_IP_ADDRESS "ls -a"
    done
