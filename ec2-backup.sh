...
delete_volume(){
    aws ec2 delete-volume --volume-id $VOLUME_ID >> backup.log
    sleep 1
}
...
dd_backup()
{
    tar cf - $SOURCE_DIR  | ssh $EC2_BACKUP_FLAGS_SSH fedora@$TARGET_IP_ADDRESS "dd of=/home/fedora/back_up.tar 1>/dev/null 2>/dev/null" >> backup.log
}

suspend_handle()
{
    #echo "In suspend_handle"
    case $PHASE in
        0) ;;
        1) terminate_instance ;;
        2) delete_volume; terminate_instance;;
        2) detach_volume; delete_volume; terminate_instance;;
        3) umount_filesystem; detach_volume; delete_volume; terminate_instance ;;
    esac
}
